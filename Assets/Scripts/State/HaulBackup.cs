...
public class HaulBackup : Job
{
    ...
    public HaulBackup(int id, Entity _target, Job _nextJob = null) : base("Haul", _nextJob)
    {
    ...
    public override void tick()
    {
        ...
        {
            jobNode = new Node(GridManager.init.GetGridCellCenter(GridManager.init.GetGridIndex(target.transform.position)));
            person.setJob(new Move(jobNode)); // Move to the item
        }
        if (isAtLoc)
        {
